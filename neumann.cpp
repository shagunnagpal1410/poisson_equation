...
int main() {
    ...
    for (int p=0; p<previous_domain.size(); p++) {
        ...
        if ((p0.x==x_min && p0.y==y_min) || (p0.x==x_min && p0.y==x_max) || (p0.x==x_max && p0.y==y_min) || (p0.x==x_max && p0.y==x_max)) {
...
answer = answer.array() - answer.mean();
    ...
    ofstream fout("Temperature1.csv");
